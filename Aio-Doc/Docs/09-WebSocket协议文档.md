...
WebSocket协议是IM系统实现实时双向通信的核心协议。本文档定义了客户端与服务端之间的WebSocket通信规范，包括连接建立、消息格式、心跳机制、异常处理等。基于STOMP协议实现消息订阅和发布，提供稳定可靠的实时通信能力。

### 核心功能

#### 1. 连接管理
- **连接建立**: 基于WebSocket建立持久连接
- **身份认证**: Token认证，确保连接安全
- **连接保活**: 心跳机制维持连接
- **断线重连**: 自动重连机制，保证服务连续性
- **多端同步**: 支持同一用户多设备同时在线
