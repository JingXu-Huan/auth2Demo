...
public class EmailVerificationController {
    ...
    public ResponseEntity<Result<Void>> sendVerificationCode(
        ...
        try {
            ...
            if (emailVerificationService.hasVerificationCode(email)) {
                ...
                return ResponseEntity.ok(Result.error(429, 
    ...
    public ResponseEntity<Result<Void>> verifyAndActivate(
            @Parameter(description = "验证请求")
            @RequestBody Map<String, String> request) {
        ...
        try {
            ...
            if (success) {
                return ResponseEntity.ok(Result.success("邮箱验证成功，账户已激活"));
            } else {
                return ResponseEntity.ok(Result.error(400, "验证失败"));
            }
     ...
     * Sentinel 限流处理
     */
    public ResponseEntity<Result<Void>> handleBlock(Map<String, String> request, 
                                                         HttpServletRequest httpRequest) {
        log.warn("请求被限流: {}", httpRequest.getRequestURI());
        return ResponseEntity.ok(Result.error(429, "请求过于频繁，请稍后重试"));
    }
}
